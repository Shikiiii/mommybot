...
async def kiss(ctx, user: discord.Member):
	embed = discord.Embed(description="**{.message.author.name}** kisses **{.name}**.".format(ctx, user), color=0xFFFFFF, timestamp=datetime.utcnow())
	embed.set_image(url=random.choice(kiss_gifs))
	embed.set_thumbnail(url="https://cdn.discordapp.com/emojis/660142158325612564.gif?v=1")
...
async def kiss_error(ctx, error):
	if isinstance(error, commands.MissingRequiredArgument):
		embed = discord.Embed(description="**Mommy** kisses **{.message.author.name}**.".format(ctx), color=0xFFFFFF, timestamp=datetime.utcnow())
		embed.set_image(url=random.choice(kiss_gifs))
		embed.set_thumbnail(url="https://cdn.discordapp.com/emojis/660142158325612564.gif?v=1")
		embed.set_footer(text="© MommyBot by Shiki.", icon_url=bot.user.avatar_url)
		await ctx.send(embed=embed)
